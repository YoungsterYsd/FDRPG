...
#include "Private/AssetTypeActions_RPGContextEffectsLibrary.h"
#include "ToolMenu.h"
#include "ToolMenus.h"
...
#define LOCTEXT_NAMESPACE "RPGEditor"

DEFINE_LOG_CATEGORY(LogRPGEditor);
...
{
	return HasNoPlayWorld() && !GetDefault<URPGDeveloperSettings>()->CommonEditorMaps.IsEmpty();
}
...
static TSharedRef<SWidget> GetCommonMapsDropdown()
{
	...
	for (const FSoftObjectPath& Path : GetDefault<URPGDeveloperSettings>()->CommonEditorMaps)
	{
		if (!Path.IsValid())
		{
			continue;
 ...
 * FLyraEditorModule
 */
class FRPGEditorModule : public FDefaultGameModuleImpl
{
	typedef FRPGEditorModule ThisClass;
	...
	virtual void StartupModule() override
	{
		...
		if (!IsRunningGame())
		{
			FModuleManager::Get().OnModulesChanged().AddRaw(this, &FRPGEditorModule::ModulesChangedCallback);
		...
		// Register the Context Effects Library asset type actions.
		{
			...
			TSharedRef<FAssetTypeActions_RPGContextEffectsLibrary> AssetAction = MakeShared<FAssetTypeActions_RPGContextEffectsLibrary>();
			RPGContextEffectsLibraryAssetAction = AssetAction;
	...
	{
		URPGExperienceManager* ExperienceManager = GEngine->GetEngineSubsystem<URPGExperienceManager>();
	...
	virtual void ShutdownModule() override
	{
		// Unregister the Context Effects Library asset type actions.
		{
			...
			TSharedPtr<IAssetTypeActions> AssetAction = RPGContextEffectsLibraryAssetAction.Pin();
...
private:
	TWeakPtr<IAssetTypeActions> RPGContextEffectsLibraryAssetAction;
