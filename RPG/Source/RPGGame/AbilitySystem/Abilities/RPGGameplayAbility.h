...
public:
	...
	UE_API URPGGameplayAbility(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category = "RPG|Ability")
	UE_API URPGAbilitySystemComponent* GetRPGAbilitySystemComponentFromActorInfo() const;

	UFUNCTION(BlueprintCallable, Category = "RPG|Ability")
	UE_API ARPGPlayerController* GetRPGPlayerControllerFromActorInfo() const;

	UFUNCTION(BlueprintCallable, Category = "RPG|Ability")
	UE_API AController* GetControllerFromActorInfo() const;

	UFUNCTION(BlueprintCallable, Category = "RPG|Ability")
	UE_API ARPGCharacter* GetRPGCharacterFromActorInfo() const;

	UFUNCTION(BlueprintCallable, Category = "RPG|Ability")
	UE_API URPGHeroComponent* GetHeroComponentFromActorInfo() const;
	...
	UE_API bool CanChangeActivationGroup(ERPGAbilityActivationGroup NewGroup) const;

	// Tries to change the activation group.  Returns true if it successfully changed.
	UFUNCTION(BlueprintCallable, BlueprintPure = false, Category = "RPG|Ability", Meta = (ExpandBoolAsExecs = "ReturnValue"))
	UE_API bool ChangeActivationGroup(ERPGAbilityActivationGroup NewGroup);

	// Sets the ability's camera mode.
	UFUNCTION(BlueprintCallable, Category = "RPG|Ability")
	UE_API void SetCameraMode(TSubclassOf<URPGCameraMode> CameraMode);
...
protected:
	...
	UE_API virtual void GetAbilitySource(FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, float& OutSourceLevel, const IRPGAbilitySourceInterface*& OutAbilitySource, AActor*& OutEffectCauser) const;
