...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RPGGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class ERPGEditorLoadMode
{
...
namespace RPGGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("RPG.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via LyraGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(URPGGameplayCueManager::DumpGameplayCues));

	static ERPGEditorLoadMode LoadMode = ERPGEditorLoadMode::LoadUpfront;
}
...
{
	switch (RPGGameplayCueManagerCvars::LoadMode)
	{
	case ERPGEditorLoadMode::LoadUpfront:
		return true;
	case ERPGEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case ERPGEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
void URPGGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	URPGGameplayCueManager* GCM = Cast<URPGGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (URPGGameplayCueManager* StrongThis = Get())
					{
...
void URPGGameplayCueManager::ProcessLoadedTags()
{
...
void URPGGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (RPGGameplayCueManagerCvars::LoadMode)
	{
	case ERPGEditorLoadMode::LoadUpfront:
...
void URPGGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void URPGGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void URPGGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
