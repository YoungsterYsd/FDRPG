...
#include "RPGAudioSettings.h"
#include "Settings/RPGSettingsLocal.h"
#include "Sound/SoundEffectSubmix.h"
#include "SoundControlBus.h"
#include "SoundControlBusMix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(RPGAudioMixEffectsSubsystem)

class FSubsystemCollectionBase;

void URPGAudioMixEffectsSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);
}

void URPGAudioMixEffectsSubsystem::Deinitialize()
{
...
bool URPGAudioMixEffectsSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
...
void URPGAudioMixEffectsSubsystem::PostInitialize()
{
	Super::PostInitialize();

	if (const URPGAudioSettings* RPGAudioSettings = GetDefault<URPGAudioSettings>())
	{
		if (UObject* ObjPath = RPGAudioSettings->DefaultControlBusMix.TryLoad())
		{
		...
		if (UObject* ObjPath = RPGAudioSettings->LoadingScreenControlBusMix.TryLoad())
		{
			if (USoundControlBusMix* SoundControlBusMix = Cast<USoundControlBusMix>(ObjPath))
			{
		...
		for (const FRPGSubmixEffectChainMap& SoftSubmixEffectChain : RPGAudioSettings->HDRAudioSubmixEffectChain)
		{
			FRPGAudioSubmixEffectsChain NewEffectChain;
		...
		for (const FRPGSubmixEffectChainMap& SoftSubmixEffectChain : RPGAudioSettings->LDRAudioSubmixEffectChain)
		{
			FRPGAudioSubmixEffectsChain NewEffectChain;
...
void URPGAudioMixEffectsSubsystem::OnWorldBeginPlay(UWorld& InWorld)
{
	...
	if (const UWorld* World = InWorld.GetWorld())
	{
		...
		if (const URPGSettingsLocal* RPGSettingsLocal = GetDefault<URPGSettingsLocal>())
		{
			...
			if (UserMix)
			{
				...
				{
					const FSoundControlBusMixStage OverallControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, OverallControlBus, RPGSettingsLocal->GetOverallVolume());
					const FSoundControlBusMixStage MusicControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, MusicControlBus, RPGSettingsLocal->GetMusicVolume());
					const FSoundControlBusMixStage SoundFXControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, SoundFXControlBus, RPGSettingsLocal->GetSoundFXVolume());
					const FSoundControlBusMixStage DialogueControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, DialogueControlBus, RPGSettingsLocal->GetDialogueVolume());
					const FSoundControlBusMixStage VoiceChatControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, VoiceChatControlBus, RPGSettingsLocal->GetVoiceChatVolume());
			...
			ApplyDynamicRangeEffectsChains(RPGSettingsLocal->IsHDRAudioModeEnabled());
		}
	}
}

void URPGAudioMixEffectsSubsystem::ApplyDynamicRangeEffectsChains(bool bHDRAudio)
{
	TArray<FRPGAudioSubmixEffectsChain> AudioSubmixEffectsChainToApply;
	TArray<FRPGAudioSubmixEffectsChain> AudioSubmixEffectsChainToClear;
	...
	for (const FRPGAudioSubmixEffectsChain& EffectChainToClear : AudioSubmixEffectsChainToClear)
	{
		bool bAddToList = true;

		for (const FRPGAudioSubmixEffectsChain& SubmixEffectChain : AudioSubmixEffectsChainToApply)
		{
...
void URPGAudioMixEffectsSubsystem::OnLoadingScreenStatusChanged(bool bShowingLoadingScreen)
{
	ApplyOrRemoveLoadingScreenMix(bShowingLoadingScreen);
}

void URPGAudioMixEffectsSubsystem::ApplyOrRemoveLoadingScreenMix(bool bWantsLoadingScreenMix)
{
...
bool URPGAudioMixEffectsSubsystem::DoesSupportWorldType(const EWorldType::Type World) const
{
