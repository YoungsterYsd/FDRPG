...
#include "RPGAudioSettings.generated.h"
...
USTRUCT()
struct FRPGSubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 * 
 */
UCLASS(MinimalAPI, config = Game, defaultconfig, meta = (DisplayName = "RPGAudioSettings"))
class URPGAudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FRPGSubmixEffectChainMap> HDRAudioSubmixEffectChain;
	
	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FRPGSubmixEffectChainMap> LDRAudioSubmixEffectChain;
