...
{
	Super::BeginPlay();
	// RPGSignificanceManager 已裁剪（C6），如需精细化显著性管理可在插件 SignificanceManager 基类上直接扩展
}
...
void ARPGCharacter::NotifyControllerChanged()
{
	...
	{
		if (IRPGTeamAgentInterface* ControllerWithTeam = Cast<IRPGTeamAgentInterface>(GetController()))
		{
...
void ARPGCharacter::InitializeGameplayTags()
{
	...
	{
		for (const TPair<uint8, FGameplayTag>& TagMapping : RPGGameplayTags::MovementModeTagMap)
		{
			if (TagMapping.Value.IsValid())
			{
				RPGASC->SetLooseGameplayTagCount(TagMapping.Value, 0);
			}
		}

		for (const TPair<uint8, FGameplayTag>& TagMapping : RPGGameplayTags::CustomMovementModeTagMap)
		{
			if (TagMapping.Value.IsValid())
			{
				RPGASC->SetLooseGameplayTagCount(TagMapping.Value, 0);
...
void ARPGCharacter::SetMovementModeTag(EMovementMode MovementMode, uint8 CustomMovementMode, bool bTagEnabled)
{
	if (URPGAbilitySystemComponent* RPGASC = GetRPGAbilitySystemComponent())
	{
		...
		{
			MovementModeTag = RPGGameplayTags::CustomMovementModeTagMap.Find(CustomMovementMode);
		}
		else
		{
			MovementModeTag = RPGGameplayTags::MovementModeTagMap.Find(MovementMode);
		}

		if (MovementModeTag && MovementModeTag->IsValid())
		{
			RPGASC->SetLooseGameplayTagCount(*MovementModeTag, (bTagEnabled ? 1 : 0));
		}
	}
}
...
void ARPGCharacter::OnStartCrouch(float HalfHeightAdjust, float ScaledHalfHeightAdjust)
{
	if (URPGAbilitySystemComponent* RPGASC = GetRPGAbilitySystemComponent())
	{
		RPGASC->SetLooseGameplayTagCount(RPGGameplayTags::Status_Crouching, 1);
	}
...
void ARPGCharacter::OnEndCrouch(float HalfHeightAdjust, float ScaledHalfHeightAdjust)
{
	if (URPGAbilitySystemComponent* RPGASC = GetRPGAbilitySystemComponent())
	{
		RPGASC->SetLooseGameplayTagCount(RPGGameplayTags::Status_Crouching, 0);
	}
...
void ARPGCharacter::SetGenericTeamId(const FGenericTeamId& NewTeamID)
{
	if (GetController() == nullptr)
	{
		...
		{
			UE_LOG(LogRPGTeams, Error, TEXT("You can't set the team ID on a character (%s) except on the authority"), *GetPathNameSafe(this));
		}
	}
	else
	{
		UE_LOG(LogRPGTeams, Error, TEXT("You can't set the team ID on a possessed character (%s); it's driven by the associated controller"), *GetPathNameSafe(this));
	}
}
...
FOnRPGTeamIndexChangedDelegate* ARPGCharacter::GetOnTeamIndexChangedDelegate()
{
	return &OnTeamChangedDelegate;
}

void ARPGCharacter::OnControllerChangedTeam(UObject* TeamAgent, int32 OldTeam, int32 NewTeam)
{
...
void ARPGCharacter::OnRep_MyTeamID(FGenericTeamId OldTeamID)
{
	ConditionalBroadcastTeamChanged(this, OldTeamID, MyTeamID);
}
...
void ARPGCharacter::FastSharedReplication_Implementation(const FSharedRepMovement& SharedRepMovement)
{
