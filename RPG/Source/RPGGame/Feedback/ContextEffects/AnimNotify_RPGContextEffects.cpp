...
#include "AnimNotify_RPGContextEffects.h"
#include "Feedback/ContextEffects/RPGContextEffectsLibrary.h"
#include "RPGContextEffectsInterface.h"
#include "Components/SkeletalMeshComponent.h"
#include "Engine/World.h"
#include "RPGContextEffectsSubsystem.h"
#include "NiagaraFunctionLibrary.h"
#include "Kismet/GameplayStatics.h"
#include "NiagaraSystem.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimNotify_RPGContextEffects)



UAnimNotify_RPGContextEffects::UAnimNotify_RPGContextEffects()
{
}

void UAnimNotify_RPGContextEffects::PostLoad()
{
	Super::PostLoad();
}

#if WITH_EDITOR
void UAnimNotify_RPGContextEffects::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
}
#endif

FString UAnimNotify_RPGContextEffects::GetNotifyName_Implementation() const
{
...
void UAnimNotify_RPGContextEffects::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	const FAnimNotifyEventReference& EventReference)
{
	...
	if (MeshComp)
	{
		...
		if (AActor* OwningActor = MeshComp->GetOwner())
		{
			...
			TArray<UObject*> RPGContextEffectImplementingObjects;

			// Determine if the Owning Actor is one of the Objects that implements the Context Effects Interface
			if (OwningActor->Implements<URPGContextEffectsInterface>())
			{
				// If so, add it to the Array
				RPGContextEffectImplementingObjects.Add(OwningActor);
			}
			...
			for (const auto Component : OwningActor->GetComponents())
			{
				if (Component)
				{
					...
					if (Component->Implements<URPGContextEffectsInterface>())
					{
						RPGContextEffectImplementingObjects.Add(Component);
					}
				}
			}

			// Cycle through all objects implementing the Context Effect Interface
			for (UObject* RPGContextEffectImplementingObject : RPGContextEffectImplementingObjects)
			{
				if (RPGContextEffectImplementingObject)
				{
					// If the object is still valid, Execute the AnimMotionEffect Event on it, passing in relevant data
					IRPGContextEffectsInterface::Execute_AnimMotionEffect(RPGContextEffectImplementingObject,
			...
			if (bPreviewInEditor)
			{
				...
				if (World && World->WorldType == EWorldType::EditorPreview)
				{
					...
					if (PreviewProperties.bPreviewPhysicalSurfaceAsContext)
					{
						...
						if (const URPGContextEffectsSettings* RPGContextEffectsSettings = GetDefault<URPGContextEffectsSettings>())
						{
							if (const FGameplayTag* SurfaceContextPtr = RPGContextEffectsSettings->SurfaceTypeToContextMap.Find(PhysicalSurfaceType))
							{
					...
					if (UObject* EffectsLibrariesObj = PreviewProperties.PreviewContextEffectsLibrary.TryLoad())
					{
						...
						if (URPGContextEffectsLibrary* EffectLibrary = Cast<URPGContextEffectsLibrary>(EffectsLibrariesObj))
						{
...
#if WITH_EDITOR
void UAnimNotify_RPGContextEffects::ValidateAssociatedAssets()
{
	Super::ValidateAssociatedAssets();
}

void UAnimNotify_RPGContextEffects::SetParameters(FGameplayTag EffectIn, FVector LocationOffsetIn, FRotator RotationOffsetIn,
	FRPGContextEffectAnimNotifyVFXSettings VFXPropertiesIn, FRPGContextEffectAnimNotifyAudioSettings AudioPropertiesIn,
	bool bAttachedIn, FName SocketNameIn, bool bPerformTraceIn, FRPGContextEffectAnimNotifyTraceSettings TracePropertiesIn)
{
	Effect = EffectIn;
