...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RPGContextEffectsLibrary)


void URPGContextEffectsLibrary::GetEffects(const FGameplayTag Effect, const FGameplayTagContainer Context, 
...
void URPGContextEffectsLibrary::LoadEffects()
{
...
EContextEffectsLibraryLoadState URPGContextEffectsLibrary::GetContextEffectsLibraryLoadState()
{
	// Return current Load State
	return EffectsLoadState;
}

void URPGContextEffectsLibrary::LoadEffectsInternal()
{
	// TODO Add Async Loading for Libraries

	// Copy data for async load
	TArray<FRPGContextEffects> LocalContextEffects = ContextEffects;

	// Prepare Active Context Effects Array
	TArray<URPGActiveContextEffects*> ActiveContextEffectsArray;

	// Loop through Context Effects
	for (const FRPGContextEffects& ContextEffect : LocalContextEffects)
	{
		// Make sure Tags are Valid
		if (ContextEffect.EffectTag.IsValid() && ContextEffect.Context.IsValid())
		{
			// Create new Active Context Effect
			URPGActiveContextEffects* NewActiveContextEffects = NewObject<URPGActiveContextEffects>(this);
	...
	this->RPGContextEffectLibraryLoadingComplete(ActiveContextEffectsArray);
}

void URPGContextEffectsLibrary::RPGContextEffectLibraryLoadingComplete(
	TArray<URPGActiveContextEffects*> RPGActiveContextEffects)
{
	// Flag data as loaded
	...
	ActiveContextEffects.Append(RPGActiveContextEffects);
}
