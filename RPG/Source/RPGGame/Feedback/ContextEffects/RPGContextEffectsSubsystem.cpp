...
#include "Feedback/ContextEffects/RPGContextEffectsSubsystem.h"
#include "Kismet/GameplayStatics.h"
#include "NiagaraFunctionLibrary.h"
#include "NiagaraSystem.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(RPGContextEffectsSubsystem)
...
void URPGContextEffectsSubsystem::SpawnContextEffects(
	...
	, float AudioPitch)
{
	...
	if (TObjectPtr<URPGContextEffectsSet>* EffectsLibrariesSetPtr = ActiveActorEffectsMap.Find(SpawningActor))
	{
		// Validate the pointers from the Map Find
		if (URPGContextEffectsSet* EffectsLibraries = *EffectsLibrariesSetPtr)
		{
			...
			for (URPGContextEffectsLibrary* EffectLibrary : EffectsLibraries->RPGContextEffectsLibraries)
			{
...
bool URPGContextEffectsSubsystem::GetContextFromSurfaceType(
	TEnumAsByte<EPhysicalSurface> PhysicalSurface, FGameplayTag& Context)
{
	// Get Project Settings
	if (const URPGContextEffectsSettings* ProjectSettings = GetDefault<URPGContextEffectsSettings>())
	{
...
void URPGContextEffectsSubsystem::LoadAndAddContextEffectsLibraries(AActor* OwningActor,
	TSet<TSoftObjectPtr<URPGContextEffectsLibrary>> ContextEffectsLibraries)
{
	...
	URPGContextEffectsSet* EffectsLibrariesSet = NewObject<URPGContextEffectsSet>(this);

	// Cycle through Libraries getting Soft Obj Refs
	for (const TSoftObjectPtr<URPGContextEffectsLibrary>& ContextEffectSoftObj : ContextEffectsLibraries)
	{
		// Load Library Assets from Soft Obj refs
		// TODO Support Async Loading of Asset Data
		if (URPGContextEffectsLibrary* EffectsLibrary = ContextEffectSoftObj.LoadSynchronous())
		{
			// Call load on valid Libraries
			EffectsLibrary->LoadEffects();

			// Add new library to Set
			EffectsLibrariesSet->RPGContextEffectsLibraries.Add(EffectsLibrary);
		}
	}
...
void URPGContextEffectsSubsystem::UnloadAndRemoveContextEffectsLibraries(AActor* OwningActor)
{
