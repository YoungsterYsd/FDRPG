...
#include "RPGNumberPopComponent_MeshText.generated.h"

class URPGDamagePopStyle;
...
class URPGNumberPopComponent_MeshText : public URPGNumberPopComponent
{
	GENERATED_BODY()

public:

	URPGNumberPopComponent_MeshText(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
...
protected:
	void SetMaterialParameters(const FRPGNumberPopRequest& Request, FTempNumberPopInfo& NewDamageNumberInfo, const FTransform& CameraTransform, const FVector& NumberLocation);

	FLinearColor DetermineColor(const FRPGNumberPopRequest& Request) const;
	UStaticMesh* DetermineStaticMesh(const FRPGNumberPopRequest& Request) const;
	...
	TArray<TObjectPtr<URPGDamagePopStyle>> Styles;
