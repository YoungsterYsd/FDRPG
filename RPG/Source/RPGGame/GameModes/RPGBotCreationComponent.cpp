...
#include "RPGBotCreationComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RPGBotCreationComponent)

URPGBotCreationComponent::URPGBotCreationComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void URPGBotCreationComponent::BeginPlay()
{
	Super::BeginPlay();
...
void URPGBotCreationComponent::OnExperienceLoaded(const URPGExperienceDefinition* Experience)
{
#if WITH_SERVER_CODE
	if (HasAuthority())
	{
...
void URPGBotCreationComponent::ServerCreateBots_Implementation()
{
	...
	if (GIsEditor)
	{
		const URPGDeveloperSettings* DeveloperSettings = GetDefault<URPGDeveloperSettings>();
...
FString URPGBotCreationComponent::CreateBotName(int32 PlayerIndex)
{
	FString Result;
...
void URPGBotCreationComponent::SpawnOneBot()
{
	...
	{
		ARPGGameMode* GameMode = GetGameMode<ARPGGameMode>();
		check(GameMode);
		...
		{
			if (URPGPawnExtensionComponent* PawnExtComponent = NewController->GetPawn()->FindComponentByClass<URPGPawnExtensionComponent>())
			{
...
void URPGBotCreationComponent::RemoveOneBot()
{
	if (SpawnedBotList.Num() > 0)
	{
		...
		if (BotToRemove)
		{
			...
			{
				if (URPGHealthComponent* HealthComponent = URPGHealthComponent::FindHealthComponent(ControlledPawn))
				{
