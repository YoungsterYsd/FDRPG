...
namespace RPGConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarExperienceLoadRandomDelayMin(
		...
		TEXT("This value (in seconds) will be added as a delay of load completion of the experience (along with the random value rpg.chaos.ExperienceDelayLoad.RandomSecs)"),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarExperienceLoadRandomDelayRange(
		...
		TEXT("A random amount of time between 0 and this value (in seconds) will be added as a delay of load completion of the experience (along with the fixed value rpg.chaos.ExperienceDelayLoad.MinSecs)"),
		ECVF_Default);
...
void URPGExperienceManagerComponent::StartExperienceLoad()
{
	...
	UE_LOG(LogRPGExperience, Log, TEXT("EXPERIENCE: StartExperienceLoad(CurrentExperience = %s, %s)"),
...
void URPGExperienceManagerComponent::OnExperienceLoadComplete()
{
	...
	UE_LOG(LogRPGExperience, Log, TEXT("EXPERIENCE: OnExperienceLoadComplete(CurrentExperience = %s, %s)"),
		*CurrentExperience->GetPrimaryAssetId().ToString(),
		*GetClientServerContextString(this));
...
#if !UE_SERVER
	URPGSettingsLocal::Get()->OnExperienceLoaded();
#endif
}
...
void URPGExperienceManagerComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	if (LoadState == ERPGExperienceLoadState::Loaded)
	{
		...
		{
			UE_LOG(LogRPGExperience, Error, TEXT("Actions that have asynchronous deactivation aren't fully supported yet in Lyra experiences"));
		}
