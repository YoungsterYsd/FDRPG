...
#include "System/RPGGameSession.h"
#include "Player/RPGPlayerController.h"
#include "Player/RPGPlayerBotController.h"
...
#include "GameModes/RPGExperienceDefinition.h"
#include "GameModes/RPGExperienceManagerComponent.h"
#include "GameModes/RPGUserFacingExperienceDefinition.h"
#include "Kismet/GameplayStatics.h"
#include "Development/RPGDeveloperSettings.h"
#include "Player/RPGPlayerSpawningManagerComponent.h"
	...
	: Super(ObjectInitializer)
{
	...
	GameSessionClass = ARPGGameSession::StaticClass();
	PlayerControllerClass = ARPGPlayerController::StaticClass();
	ReplaySpectatorPlayerControllerClass = ARPGReplayPlayerController::StaticClass();
...
void ARPGGameMode::HandleMatchAssignmentIfNotExpectingOne()
{
	...
	if (!ExperienceId.IsValid())
	{
		...
		ExperienceId = FPrimaryAssetId(FPrimaryAssetType(URPGExperienceDefinition::StaticClass()->GetFName()), FName("B_RPG_Experience_FrontEnd_Menu"));
...
bool ARPGGameMode::TryDedicatedServerLogin()
{
	...
	if (GameInstance && World && World->GetNetMode() == NM_DedicatedServer && World->URL.Map == DefaultMap)
	{
		...
		UserSubsystem->OnUserInitializeComplete.AddDynamic(this, &ARPGGameMode::OnUserInitializedForDedicatedServer);
...
void ARPGGameMode::HostDedicatedServerMatch(ECommonSessionOnlineMode OnlineMode)
{
	FPrimaryAssetType UserExperienceType = URPGUserFacingExperienceDefinition::StaticClass()->GetFName();
	...
	URPGUserFacingExperienceDefinition* FoundExperience = nullptr;
	URPGUserFacingExperienceDefinition* DefaultExperience = nullptr;

	for (UObject* Object : UserExperiences)
	{
		URPGUserFacingExperienceDefinition* UserExperience = Cast<URPGUserFacingExperienceDefinition>(Object);
...
void ARPGGameMode::OnUserInitializedForDedicatedServer(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	UGameInstance* GameInstance = GetGameInstance();
	if (GameInstance)
	{
		// Unbind
		UCommonUserSubsystem* UserSubsystem = GameInstance->GetSubsystem<UCommonUserSubsystem>();
		UserSubsystem->OnUserInitializeComplete.RemoveDynamic(this, &ARPGGameMode::OnUserInitializedForDedicatedServer);

		// Dedicated servers do not require user login, but some online subsystems may expect it
		if (bSuccess && ensure(UserInfo))
		{
			UE_LOG(LogRPGExperience, Log, TEXT("Dedicated server user login succeeded for id %s, starting online server"), *UserInfo->GetNetId().ToString());
		}
		else
		{
			UE_LOG(LogRPGExperience, Log, TEXT("Dedicated server user login unsuccessful, starting online server as login is not required"));
		}
...
void ARPGGameMode::OnMatchAssignmentGiven(FPrimaryAssetId ExperienceId, const FString& ExperienceIdSource)
{
	if (ExperienceId.IsValid())
	{
		...
		// 防御性校验：在调 SetCurrentExperience 前先确认 AssetManager 真的能找到这个资产
		// （避免资产因 PrimaryAssetType 冲突被 Ignore 时，进入 SetCurrentExperience 触发 check(AssetClass) 致命崩溃）
		URPGAssetManager& AssetManager = URPGAssetManager::Get();
		FAssetData Probe;
		if (!AssetManager.GetPrimaryAssetData(ExperienceId, /*out*/ Probe))
		{
			UE_LOG(LogRPGExperience, Error,
				TEXT("Experience %s claimed valid but AssetManager has NO AssetData for it. "
				     "Likely cause: the asset's serialized PrimaryAssetType conflicts with current C++ class name "
				     "(check AssetManager 'Ignoring PrimaryAssetType' warning above). "
				     "Aborting load to prevent fatal assertion. Source: %s"),
...
{
	if (URPGPlayerSpawningManagerComponent* PlayerSpawningComponent = GameState->FindComponentByClass<URPGPlayerSpawningManagerComponent>())
	{
...
void ARPGGameMode::RequestPlayerRestartNextFrame(AController* Controller, bool bForceReset)
{
	...
	else if (ARPGPlayerBotController* BotController = Cast<ARPGPlayerBotController>(Controller))
	{
		GetWorldTimerManager().SetTimerForNextTick(BotController, &ARPGPlayerBotController::ServerRestartController);
	}
}
