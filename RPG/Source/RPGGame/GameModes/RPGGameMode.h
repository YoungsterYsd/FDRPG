...
class ARPGGameMode : public AModularGameModeBase
{
	GENERATED_BODY()

public:

	UE_API ARPGGameMode(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category = "RPG|Pawn")
	UE_API const URPGPawnData* GetPawnDataForController(const AController* InController) const;
