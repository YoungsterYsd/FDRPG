...
class ARPGGameState : public AModularGameStateBase, public IAbilitySystemInterface
{
	GENERATED_BODY()

public:

	UE_API ARPGGameState(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	UE_API void MulticastMessageToClients(const FRPGVerbMessage Message);

	// Send a message that all clients will be guaranteed to get
	// (use only for client notifications that cannot handle being lost)
	UFUNCTION(NetMulticast, Reliable, BlueprintCallable, Category = "RPG|GameState")
	UE_API void MulticastReliableMessageToClients(const FRPGVerbMessage Message);
