...
#include "RPGPerformanceSettings.generated.h"
...
USTRUCT()
struct FRPGQualityDeviceProfileVariant
{
	GENERATED_BODY()
...
USTRUCT()
struct FRPGPerformanceStatGroup
{
	GENERATED_BODY()
...
class URPGPlatformSpecificRenderingSettings : public UPlatformSettings
{
	GENERATED_BODY()

public:
	URPGPlatformSpecificRenderingSettings();

	// Helper method to get the performance settings object, directed via platform settings
	static const URPGPlatformSpecificRenderingSettings* Get();
...
public:
	...
	// Note that this will usually be set from platform-specific ini files, not via the UI
	UPROPERTY(EditAnywhere, Config, Category=DeviceProfiles)
	TArray<FRPGQualityDeviceProfileVariant> UserFacingDeviceProfileOptions;
	...
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings)
	ERPGFramePacingMode FramePacingMode = ERPGFramePacingMode::DesktopStyle;

	// Potential frame rates to display for mobile
	// Note: This is further limited by RPG.DeviceProfile.Mobile.MaxFrameRate from the
 ...
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="RPG Performance Settings"))
class URPGPerformanceSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	URPGPerformanceSettings();
...
public:
	...
	TArray<FRPGPerformanceStatGroup> UserFacingPerformanceStats;
};
