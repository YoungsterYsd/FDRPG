 ...
 */
UCLASS(MinimalAPI)
class URPGLocalPlayer : public UCommonLocalPlayer, public IRPGTeamAgentInterface
{
	GENERATED_BODY()

public:

	UE_API URPGLocalPlayer();
	...
	UFUNCTION()
	UE_API URPGSettingsLocal* GetLocalSettings() const;

	/** Gets the shared setting for this player, this is read using the save game system so may not be correct until after user login */
	UFUNCTION()
	UE_API URPGSettingsShared* GetSharedSettings() const;
...
protected:
	UE_API void OnSharedSettingsLoaded(URPGSettingsShared* LoadedOrCreatedSettings);
...
private:
	UPROPERTY(Transient)
	mutable TObjectPtr<URPGSettingsShared> SharedSettings;
