...
enum class ERPGPlayerStartLocationOccupancy
{
	Empty,
	Partial,
	Full
...
class ARPGPlayerStart : public APlayerStart
{
	GENERATED_BODY()

public:
	UE_API ARPGPlayerStart(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	const FGameplayTagContainer& GetGameplayTags() { return StartPointTags; }

	UE_API ERPGPlayerStartLocationOccupancy GetLocationOccupancy(AController* const ControllerPawnToFit) const;
