...
class ARPGPlayerState : public AModularPlayerState, public IAbilitySystemInterface, public IRPGTeamAgentInterface
{
	GENERATED_BODY()

public:
	UE_API ARPGPlayerState(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	UE_API void SetPawnData(const URPGPawnData* InPawnData);
	...
	static UE_API const FName NAME_RPGAbilityReady;

	UE_API void SetPlayerConnectionType(ERPGPlayerConnectionType NewType);
	...
	UFUNCTION(Client, Unreliable, BlueprintCallable, Category = "RPG|PlayerState")
	UE_API void ClientBroadcastMessage(const FRPGVerbMessage Message);
...
private:
	UE_API void OnExperienceLoaded(const URPGExperienceDefinition* CurrentExperience);
