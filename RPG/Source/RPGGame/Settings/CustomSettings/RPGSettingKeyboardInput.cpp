...
#include "RPGSettingKeyboardInput.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RPGSettingKeyboardInput)
...
URPGSettingKeyboardInput::URPGSettingKeyboardInput()
{
	bReportAnalytics = false;
}

FText URPGSettingKeyboardInput::GetSettingDisplayName() const
{
...
FText URPGSettingKeyboardInput::GetSettingDisplayCategory() const
{
	if (const FKeyMappingRow* Row = FindKeyMappingRow())
	{
		if (Row->HasAnyMappings())
...
const FKeyMappingRow* URPGSettingKeyboardInput::FindKeyMappingRow() const
{
...
UEnhancedPlayerMappableKeyProfile* URPGSettingKeyboardInput::FindMappableKeyProfile() const
{
...
UEnhancedInputUserSettings* URPGSettingKeyboardInput::GetUserSettings() const
{
	if(URPGLocalPlayer* RPGLocalPlayer = Cast<URPGLocalPlayer>(LocalPlayer))
	{
		// Map the key to the player key profile
		if (UEnhancedInputLocalPlayerSubsystem* System = RPGLocalPlayer->GetSubsystem<UEnhancedInputLocalPlayerSubsystem>())
		{
...
void URPGSettingKeyboardInput::OnInitialized()
{
...
void URPGSettingKeyboardInput::InitializeInputData(const UEnhancedPlayerMappableKeyProfile* KeyProfile, const FKeyMappingRow& MappingData, const FPlayerMappableKeyQueryOptions& InQueryOptions)
{
	check(KeyProfile);
...
FText URPGSettingKeyboardInput::GetKeyTextFromSlot(const EPlayerMappableKeySlot InSlot) const
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
...
void URPGSettingKeyboardInput::ResetToDefault()
{	
	if (UEnhancedInputUserSettings* Settings = GetUserSettings())
	{
...
void URPGSettingKeyboardInput::StoreInitial()
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
		if(const FKeyMappingRow* Row = FindKeyMappingRow())
...
void URPGSettingKeyboardInput::RestoreToInitial()
{
...
bool URPGSettingKeyboardInput::ChangeBinding(int32 InKeyBindSlot, FKey NewKey)
{
...
void URPGSettingKeyboardInput::GetAllMappedActionsFromKey(int32 InKeyBindSlot, FKey Key, TArray<FName>& OutActionNames) const
{	
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{		
...
bool URPGSettingKeyboardInput::IsMappingCustomized() const
{
	bool bResult = false;
