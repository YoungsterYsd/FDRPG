...
#include "Audio/RPGAudioSettings.h"
#include "Audio/RPGAudioMixEffectsSubsystem.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(RPGSettingsLocal)
...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("RPG.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("RPG.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("RPG.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("RPG.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("RPG.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("RPG.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("RPG.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("RPG.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("RPG.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("RPG.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FRPGScalabilitySnapshot::FRPGScalabilitySnapshot()
{
...
namespace RPGSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FRPGScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
	...
	int32 ConstrainFrameRateToBeCompatibleWithOverallQuality(int32 FrameRate, int32 OverallQuality)
	{
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = URPGSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : URPGSettingsLocal::GetDefaultMobileFrameRate();
	}
...
URPGSettingsLocal::URPGSettingsLocal()
{
	...
	bEnableScalabilitySettings = URPGPlatformSpecificRenderingSettings::Get()->bSupportsGranularVideoQualitySettings;

	SetToDefaults();
}

void URPGSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
	...
	bEnableLatencyTrackingStats = URPGSettingsLocal::DoesPlatformSupportLatencyTrackingStats();
...
void URPGSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const URPGPlatformSpecificRenderingSettings* PlatformSettings = URPGPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ERPGFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	RPGSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void URPGSettingsLocal::ResetToCurrentSettings()
{
...
void URPGSettingsLocal::BeginDestroy()
{
...
URPGSettingsLocal* URPGSettingsLocal::Get()
{
	return GEngine ? CastChecked<URPGSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void URPGSettingsLocal::ConfirmVideoMode()
{
...
float URPGSettingsLocal::GetEffectiveFrameRateLimit()
{
	const URPGPlatformSpecificRenderingSettings* PlatformSettings = URPGPlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == ERPGFramePacingMode::DesktopStyle)
	{
...
int32 URPGSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return RPGSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void URPGSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FRPGScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void URPGSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void URPGSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void URPGSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void URPGSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void URPGSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool URPGSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
ERPGStatDisplayMode URPGSettingsLocal::GetPerfStatDisplayState(ERPGDisplayablePerformanceStat Stat) const
{
	if (const ERPGStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return ERPGStatDisplayMode::Hidden;
	}
}

void URPGSettingsLocal::SetPerfStatDisplayState(ERPGDisplayablePerformanceStat Stat, ERPGStatDisplayMode DisplayMode)
{
	if (DisplayMode == ERPGStatDisplayMode::Hidden)
	{
...
bool URPGSettingsLocal::DoesPlatformSupportLatencyMarkers()
{
	return ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(PerfStatTags::TAG_Platform_Trait_SupportsLatencyMarkers);
}

void URPGSettingsLocal::SetEnableLatencyFlashIndicators(const bool bNewVal)
{
...
void URPGSettingsLocal::SetEnableLatencyTrackingStats(const bool bNewVal)
{
...
void URPGSettingsLocal::ApplyLatencyTrackingStatSetting()
{
...
bool URPGSettingsLocal::DoesPlatformSupportLatencyTrackingStats()
{
	return ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(PerfStatTags::TAG_Platform_Trait_SupportsLatencyStats);
}

float URPGSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void URPGSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void URPGSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float URPGSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void URPGSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float URPGSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void URPGSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float URPGSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void URPGSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float URPGSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void URPGSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void URPGSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 URPGSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 URPGSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool URPGSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 URPGSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return RPGSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 URPGSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return RPGSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void URPGSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 URPGSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const URPGPlatformSpecificRenderingSettings* PlatformSettings = URPGPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == ERPGFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return RPGSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void URPGSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const URPGPlatformSpecificRenderingSettings* PlatformSettings = URPGPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ERPGFramePacingMode::MobileStyle)
	{
...
void URPGSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void URPGSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const URPGPlatformSpecificRenderingSettings* PlatformSettings = URPGPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ERPGFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = RPGSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void URPGSettingsLocal::ClampMobileQuality()
{
	const URPGPlatformSpecificRenderingSettings* PlatformSettings = URPGPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ERPGFramePacingMode::MobileStyle)
	{
...
void URPGSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = RPGSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void URPGSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = RPGSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = RPGSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString URPGSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void URPGSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void URPGSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool URPGSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool URPGSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = RPGSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool URPGSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void URPGSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
			if (URPGAudioMixEffectsSubsystem* RPGAudioMixEffectsSubsystem = World->GetSubsystem<URPGAudioMixEffectsSubsystem>())
			{
				RPGAudioMixEffectsSubsystem->ApplyDynamicRangeEffectsChains(bEnabled);
			}
		}
	}
}

bool URPGSettingsLocal::CanRunAutoBenchmark() const
{
	const URPGPlatformSpecificRenderingSettings* PlatformSettings = URPGPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool URPGSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void URPGSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void URPGSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float URPGSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void URPGSettingsLocal::SetOverallVolume(float InVolume)
{
...
float URPGSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void URPGSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float URPGSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void URPGSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float URPGSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void URPGSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float URPGSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void URPGSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void URPGSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void URPGSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void URPGSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void URPGSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 URPGSettingsLocal::GetOverallScalabilityLevel() const
{
...
void URPGSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == ERPGFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = RPGSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void URPGSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName URPGSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void URPGSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const URPGAudioSettings* RPGAudioSettings = GetDefault<URPGAudioSettings>())
			{
				...
				if (UObject* ObjPath = RPGAudioSettings->OverallVolumeControlBus.TryLoad())
				{
				...
				if (UObject* ObjPath = RPGAudioSettings->MusicVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
				...
				if (UObject* ObjPath = RPGAudioSettings->SoundFXVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
				...
				if (UObject* ObjPath = RPGAudioSettings->DialogueVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
				...
				if (UObject* ObjPath = RPGAudioSettings->VoiceChatVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
				...
				if (UObject* ObjPath = RPGAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
...
void URPGSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void URPGSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const URPGPlatformSpecificRenderingSettings* PlatformSettings = URPGPlatformSpecificRenderingSettings::Get();
	const TArray<FRPGQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FRPGQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	// URPGPlatformEmulationSettings 已裁剪（C5），去掉编辑器里模拟 Android/iOS 设备 Profile 的能力
	...
	{
	case ERPGFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case ERPGFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case ERPGFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void URPGSettingsLocal::UpdateConsoleFramePacing()
{
...
void URPGSettingsLocal::UpdateDesktopFramePacing()
{
...
void URPGSettingsLocal::UpdateMobileFramePacing()
{
...
void URPGSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
