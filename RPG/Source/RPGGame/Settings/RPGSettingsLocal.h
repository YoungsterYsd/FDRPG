...
#include "RPGSettingsLocal.generated.h"
...
USTRUCT()
struct FRPGScalabilitySnapshot
{
	GENERATED_BODY()

	FRPGScalabilitySnapshot();
 ...
 */
UCLASS()
class URPGSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	ERPGStatDisplayMode GetPerfStatDisplayState(ERPGDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(ERPGDisplayablePerformanceStat Stat, ERPGStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(URPGSettingsLocal, FPerfStatSettingsChanged);
	...
	DECLARE_EVENT(URPGSettingsLocal, FLatencyFlashInidicatorSettingChanged);
	UFUNCTION()
	...
	DECLARE_EVENT(URPGSettingsLocal, FLatencyStatEnabledSettingChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<ERPGDisplayablePerformanceStat, ERPGStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FRPGScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FRPGScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(URPGSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
