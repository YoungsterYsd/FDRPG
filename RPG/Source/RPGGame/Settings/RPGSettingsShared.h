...
#include "RPGSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class ERPGAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
 ...
 */
UCLASS()
class URPGSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(URPGSettingsShared, FOnSettingChangedEvent, URPGSettingsShared* Settings);
...
public:
	...
	static URPGSettingsShared* CreateTemporarySettings(const URPGLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static URPGSettingsShared* LoadOrCreateSettings(const URPGLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, URPGSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const URPGLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
private:
	...
	////////////////////////////////////////////////////////
	// Subtitles 已裁剪（P2 GameSubtitles 插件移除）

	////////////////////////////////////////////////////////
	// Shared audio settings
public:
	UFUNCTION()
	ERPGAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(ERPGAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	ERPGAllowBackgroundAudioSetting AllowAudioInBackground = ERPGAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	ERPGGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(ERPGGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	ERPGGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(ERPGGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	ERPGGamepadSensitivity GamepadLookSensitivityPreset = ERPGGamepadSensitivity::Normal;
	UPROPERTY()
	ERPGGamepadSensitivity GamepadTargetingSensitivityPreset = ERPGGamepadSensitivity::Normal;
