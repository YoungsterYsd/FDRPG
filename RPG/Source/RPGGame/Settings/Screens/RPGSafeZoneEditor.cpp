...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RPGSafeZoneEditor)
...
URPGSafeZoneEditor::URPGSafeZoneEditor(const FObjectInitializer& Initializer)
	: Super(Initializer)
{
...
void URPGSafeZoneEditor::NativeOnInitialized()
{
	Super::NativeOnInitialized();
	Switcher_SafeZoneMessage->SetActiveWidget(RichText_Default);
}

void URPGSafeZoneEditor::NativeOnActivated()
{
	Super::NativeOnActivated();

	SSafeZone::SetGlobalSafeZoneScale(URPGSettingsLocal::Get()->GetSafeZone());
	
	Button_Done->OnClicked().AddUObject(this, &URPGSafeZoneEditor::HandleDoneClicked);

	Button_Back->SetVisibility((bCanCancel)? ESlateVisibility::Visible : ESlateVisibility::Collapsed);
	if (bCanCancel)
	{
		Button_Back->OnClicked().AddUObject(this, &URPGSafeZoneEditor::HandleBackClicked);
	}
}

bool URPGSafeZoneEditor::ExecuteActionForSetting_Implementation(FGameplayTag ActionTag, UGameSetting* InSetting)
{
...
FReply URPGSafeZoneEditor::NativeOnAnalogValueChanged(const FGeometry& InGeometry, const FAnalogInputEvent& InAnalogEvent)
{
...
FReply URPGSafeZoneEditor::NativeOnMouseWheel(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
...
void URPGSafeZoneEditor::HandleInputModeChanged(ECommonInputType InInputType)
{
...
void URPGSafeZoneEditor::HandleBackClicked()
{
	DeactivateWidget();
	SSafeZone::SetGlobalSafeZoneScale(URPGSettingsLocal::Get()->GetSafeZone());
}

void URPGSafeZoneEditor::HandleDoneClicked()
{
	...
	{
		URPGSettingsLocal::Get()->SetSafeZone(SSafeZone::GetGlobalSafeZoneScale().Get(1.0f));
	}
