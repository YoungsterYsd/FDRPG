...
namespace Lyra
{
	...
	static FAutoConsoleVariableRef CVarRPGTestEncryption(
		TEXT("RPG.TestEncryption"),
		bTestEncryption,
	...
	static FAutoConsoleVariableRef CVarRPGUseDTLSEncryption(
		TEXT("RPG.UseDTLSEncryption"),
		bUseDTLSEncryption,
		TEXT("Set to true if using RPG.TestEncryption and the DTLS packet handler."),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarRPGTestDTLSFingerprint(
		TEXT("RPG.TestDTLSFingerprint"),
...
void URPGGameInstance::Init()
{
	...
	{
		SessionSubsystem->OnPreClientTravelEvent.AddUObject(this, &URPGGameInstance::OnPreClientTravelToSession);
	}
}
...
ARPGPlayerController* URPGGameInstance::GetPrimaryPlayerController() const
{
	return Cast<ARPGPlayerController>(Super::GetPrimaryPlayerController(false));
}

bool URPGGameInstance::CanJoinRequestedSession() const
{
...
void URPGGameInstance::HandlerUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		URPGLocalPlayer* LocalPlayer = Cast<URPGLocalPlayer>(GetLocalPlayerByIndex(UserInfo->LocalPlayerIndex));
...
void URPGGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
...
void URPGGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
...
void URPGGameInstance::OnPreClientTravelToSession(FString& URL)
{
