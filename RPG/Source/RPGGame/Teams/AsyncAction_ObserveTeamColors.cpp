...
#include "Teams/RPGTeamStatics.h"
...
UAsyncAction_ObserveTeamColors* UAsyncAction_ObserveTeamColors::ObserveTeamColors(UObject* TeamAgent)
{
	...
	if (TeamAgent != nullptr)
	{
		...
		Action->TeamInterfacePtr = TWeakInterfacePtr<IRPGTeamAgentInterface>(TeamAgent);
...
void UAsyncAction_ObserveTeamColors::SetReadyToDestroy()
{
	...
	if (IRPGTeamAgentInterface* TeamInterface = TeamInterfacePtr.Get())
	{
...
void UAsyncAction_ObserveTeamColors::Activate()
{
	...
	URPGTeamDisplayAsset* CurrentDisplayAsset = nullptr;
	...
	if (IRPGTeamAgentInterface* TeamInterface = TeamInterfacePtr.Get())
	{
		if (UWorld* World = GEngine->GetWorldFromContextObject(TeamInterfaceObj.Get(), EGetWorldErrorMode::LogAndReturnNull))
		{
			...
			CurrentDisplayAsset = URPGTeamStatics::GetTeamDisplayAsset(World, CurrentTeamIndex);
...
void UAsyncAction_ObserveTeamColors::BroadcastChange(int32 NewTeam, const URPGTeamDisplayAsset* DisplayAsset)
{
	UWorld* World = GEngine->GetWorldFromContextObject(TeamInterfaceObj.Get(), EGetWorldErrorMode::LogAndReturnNull);
	URPGTeamSubsystem* TeamSubsystem = UWorld::GetSubsystem<URPGTeamSubsystem>(World);
...
{
	URPGTeamDisplayAsset* DisplayAsset = URPGTeamStatics::GetTeamDisplayAsset(TeamAgent, NewTeam);
	BroadcastChange(NewTeam, DisplayAsset);
}

void UAsyncAction_ObserveTeamColors::OnDisplayAssetChanged(const URPGTeamDisplayAsset* DisplayAsset)
{
