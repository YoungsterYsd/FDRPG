...
class IRPGTeamAgentInterface;
class URPGTeamDisplayAsset;
struct FFrame;

DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FTeamColorObservedAsyncDelegate, bool, bTeamSet, int32, TeamId, const URPGTeamDisplayAsset*, DisplayAsset);
...
private:
	void BroadcastChange(int32 NewTeam, const URPGTeamDisplayAsset* DisplayAsset);

	UFUNCTION()
	void OnWatchedAgentChangedTeam(UObject* TeamAgent, int32 OldTeam, int32 NewTeam);

	UFUNCTION()
	void OnDisplayAssetChanged(const URPGTeamDisplayAsset* DisplayAsset);

	TWeakInterfacePtr<IRPGTeamAgentInterface> TeamInterfacePtr;
