...
#include "RPGTeamSubsystem.generated.h"
...
public:
	UPROPERTY()
	TObjectPtr<ARPGTeamPublicInfo> PublicInfo = nullptr;

	UPROPERTY()
	TObjectPtr<ARPGTeamPrivateInfo> PrivateInfo = nullptr;
...
public:
	void SetTeamInfo(ARPGTeamInfoBase* Info);
	void RemoveTeamInfo(ARPGTeamInfoBase* Info);
};
...
public:
	...
	UE_API bool RegisterTeamInfo(ARPGTeamInfoBase* TeamInfo);

	// Tries to unregister a team, will return false if it didn't work
	UE_API bool UnregisterTeamInfo(ARPGTeamInfoBase* TeamInfo);
	...
	UE_API const ARPGPlayerState* FindPlayerStateFromActor(const AActor* PossibleTeamActor) const;
	...
	UE_API ERPGTeamComparison CompareTeams(const UObject* A, const UObject* B, int32& TeamIdA, int32& TeamIdB) const;

	// Compare the teams of two actors and returns a value indicating if they are on same teams, different teams, or one/both are invalid
	UE_API ERPGTeamComparison CompareTeams(const UObject* A, const UObject* B) const;
	...
	UFUNCTION(BlueprintCallable, Category=Teams)
	UE_API URPGTeamDisplayAsset* GetTeamDisplayAsset(int32 TeamId, int32 ViewerTeamId);

	// Gets the team display asset for the specified team, from the perspective of the specified team
	// (You have to specify a viewer too, in case the game mode is in a 'local player is always blue team' sort of situation)
	UFUNCTION(BlueprintCallable, Category = Teams)
	UE_API URPGTeamDisplayAsset* GetEffectiveTeamDisplayAsset(int32 TeamId, UObject* ViewerTeamAgent);
	...
	UE_API void NotifyTeamDisplayAssetModified(URPGTeamDisplayAsset* ModifiedAsset);

	// Register for a team display asset notification for the specified team ID
	UE_API FOnRPGTeamDisplayAssetChangedDelegate& GetTeamDisplayAssetChangedDelegate(int32 TeamId);

private:
	UPROPERTY()
	TMap<int32, FRPGTeamTrackingInfo> TeamMap;
