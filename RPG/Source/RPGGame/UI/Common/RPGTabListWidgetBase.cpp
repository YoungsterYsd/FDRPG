...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RPGTabListWidgetBase)

void URPGTabListWidgetBase::NativeOnInitialized()
{
	Super::NativeOnInitialized();
}

void URPGTabListWidgetBase::NativeConstruct()
{
...
void URPGTabListWidgetBase::NativeDestruct()
{
	for (FRPGTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool URPGTabListWidgetBase::GetPreregisteredTabInfo(const FName TabNameId, FRPGTabDescriptor& OutTabInfo)
{
	const FRPGTabDescriptor* const FoundTabInfo = PreregisteredTabInfoArray.FindByPredicate([&](FRPGTabDescriptor& TabInfo) -> bool
	{
...
void URPGTabListWidgetBase::SetTabHiddenState(FName TabNameId, bool bHidden)
{
	for (FRPGTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool URPGTabListWidgetBase::RegisterDynamicTab(const FRPGTabDescriptor& TabDescriptor)
{
...
void URPGTabListWidgetBase::HandlePreLinkedSwitcherChanged()
{
	for (const FRPGTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void URPGTabListWidgetBase::HandlePostLinkedSwitcherChanged()
{
...
void URPGTabListWidgetBase::HandleTabCreation_Implementation(FName TabId, UCommonButtonBase* TabButton)
{
	FRPGTabDescriptor* TabInfoPtr = nullptr;
	
	FRPGTabDescriptor TabInfo;
	...
	if (TabButton->GetClass()->ImplementsInterface(URPGTabButtonInterface::StaticClass()))
	{
		...
		{
			IRPGTabButtonInterface::Execute_SetTabLabelInfo(TabButton, *TabInfoPtr);
		}
	}
...
bool URPGTabListWidgetBase::IsFirstTabActive() const
{
...
bool URPGTabListWidgetBase::IsLastTabActive() const
{
	if (PreregisteredTabInfoArray.Num() > 0)
	{
...
bool URPGTabListWidgetBase::IsTabVisible(FName TabId)
{
...
int32 URPGTabListWidgetBase::GetVisibleTabCount()
{
	int32 Result = 0;
...
void URPGTabListWidgetBase::SetupTabs()
{
	for (FRPGTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		if (TabInfo.bHidden)
