...
#include "RPGFrontendStateComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RPGFrontendStateComponent)
...
URPGFrontendStateComponent::URPGFrontendStateComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void URPGFrontendStateComponent::BeginPlay()
{
	Super::BeginPlay();
...
void URPGFrontendStateComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	Super::EndPlay(EndPlayReason);
}

bool URPGFrontendStateComponent::ShouldShowLoadingScreen(FString& OutReason) const
{
...
void URPGFrontendStateComponent::OnExperienceLoaded(const URPGExperienceDefinition* Experience)
{
...
void URPGFrontendStateComponent::FlowStep_WaitForUserInitialization(FControlFlowNodeRef SubFlow)
{
...
void URPGFrontendStateComponent::FlowStep_TryShowPressStartScreen(FControlFlowNodeRef SubFlow)
{
	...
	if (!UserSubsystem->ShouldWaitForStartInput())
	{
		...
		UserSubsystem->OnUserInitializeComplete.AddDynamic(this, &URPGFrontendStateComponent::OnUserInitialized);
...
void URPGFrontendStateComponent::OnUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UserSubsystem->OnUserInitializeComplete.RemoveDynamic(this, &URPGFrontendStateComponent::OnUserInitialized);
...
void URPGFrontendStateComponent::FlowStep_TryJoinRequestedSession(FControlFlowNodeRef SubFlow)
{
...
void URPGFrontendStateComponent::FlowStep_TryShowMainScreen(FControlFlowNodeRef SubFlow)
{
	if (UPrimaryGameLayout* RootLayout = UPrimaryGameLayout::GetPrimaryGameLayoutForPrimaryPlayer(this))
	{
		constexpr bool bSuspendInputUntilComplete = true;
