...
#include "RPGReticleWidgetBase.h"

#include "Inventory/RPGInventoryItemInstance.h"
#include "Weapons/RPGRangedWeaponInstance.h"
#include "Weapons/RPGWeaponInstance.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(RPGReticleWidgetBase)

URPGReticleWidgetBase::URPGReticleWidgetBase(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void URPGReticleWidgetBase::InitializeFromWeapon(URPGWeaponInstance* InWeapon)
{
	WeaponInstance = InWeapon;
	InventoryInstance = nullptr;
	if (WeaponInstance)
	{
		InventoryInstance = Cast<URPGInventoryItemInstance>(WeaponInstance->GetInstigator());
	}
	OnWeaponInitialized();
}


float URPGReticleWidgetBase::ComputeSpreadAngle() const
{
	if (const URPGRangedWeaponInstance* RangedWeapon = Cast<const URPGRangedWeaponInstance>(WeaponInstance))
	{
...
bool URPGReticleWidgetBase::HasFirstShotAccuracy() const
{
	if (const URPGRangedWeaponInstance* RangedWeapon = Cast<const URPGRangedWeaponInstance>(WeaponInstance))
	{
...
float URPGReticleWidgetBase::ComputeMaxScreenspaceSpreadRadius() const
{
