...
#include "RPGGameplayAbility_RangedWeapon.h"
#include "Weapons/RPGRangedWeaponInstance.h"
#include "Physics/RPGCollisionChannels.h"
#include "RPGLogChannels.h"
#include "AIController.h"
#include "NativeGameplayTags.h"
#include "Weapons/RPGWeaponStateComponent.h"
#include "AbilitySystemComponent.h"
#include "AbilitySystem/RPGGameplayAbilityTargetData_SingleTargetHit.h"
#include "DrawDebugHelpers.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(RPGGameplayAbility_RangedWeapon)
...
namespace RPGConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarDrawBulletTraceDuraton(
		TEXT("rpg.Weapon.DrawBulletTraceDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHits(
		TEXT("rpg.Weapon.DrawBulletHitDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHitRadius(
		TEXT("rpg.Weapon.DrawBulletHitRadius"),
		DrawBulletHitRadius,
...
URPGGameplayAbility_RangedWeapon::URPGGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

URPGRangedWeaponInstance* URPGGameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<URPGRangedWeaponInstance>(GetAssociatedEquipment());
}

bool URPGGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(LogRPGAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*URPGRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 URPGGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void URPGGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel URPGGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return RPG_TraceChannel_Weapon;
}

FHitResult URPGGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector URPGGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform URPGGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, ERPGAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	URPGWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<URPGWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != ERPGAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == ERPGAbilityTargetingSource::CameraTowardsFocus) || (Source == ERPGAbilityTargetingSource::PawnTowardsFocus) || (Source == ERPGAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == ERPGAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == ERPGAbilityTargetingSource::WeaponForward) || (Source == ERPGAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == ERPGAbilityTargetingSource::PawnTowardsFocus) || (Source == ERPGAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult URPGGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (RPGConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, RPGConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void URPGGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	URPGRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, ERPGAbilityTargetingSource::CameraTowardsFocus);
		...
		if (RPGConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, RPGConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void URPGGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	URPGRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (RPGConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, RPGConsoleVariables::DrawBulletHitRadius, FColor::Red, false, RPGConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void URPGGameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void URPGGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void URPGGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (URPGWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<URPGWeaponStateComponent>())
					{
		...
		{
			UE_LOG(LogRPGAbilitySystem, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void URPGGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	URPGWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<URPGWeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FRPGGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FRPGGameplayAbilityTargetData_SingleTargetHit();
