...
#include "RPGGameplayAbility_RangedWeapon.generated.h"

enum ECollisionChannel : int;

class APawn;
class URPGRangedWeaponInstance;
class UObject;
...
UENUM(BlueprintType)
enum class ERPGAbilityTargetingSource : uint8
{
 ...
 */
UCLASS()
class URPGGameplayAbility_RangedWeapon : public URPGGameplayAbility_FromEquipment
{
	GENERATED_BODY()

public:

	URPGGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category = "RPG|Ability")
	URPGRangedWeaponInstance* GetWeaponInstance() const;
...
protected:
	struct FRangedWeaponFiringInput
	{
		...
		URPGRangedWeaponInstance* WeaponData = nullptr;
...
protected:
	...
	FTransform GetTargetingTransform(APawn* SourcePawn, ERPGAbilityTargetingSource Source) const;
