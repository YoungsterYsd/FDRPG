	...
	: Super(ObjectInitializer)
{
	...
	if (APawn* Pawn = GetPawn())
	{
		...
		{
			if (URPGHealthComponent* HealthComponent = URPGHealthComponent::FindHealthComponent(GetPawn()))
			{
...
void URPGWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void URPGWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
...
void URPGWeaponInstance::UpdateFiringTime()
{
	UWorld* World = GetWorld();
	check(World);
...
float URPGWeaponInstance::GetTimeSinceLastInteractedWith() const
{
	UWorld* World = GetWorld();
	check(World);
...
TSubclassOf<UAnimInstance> URPGWeaponInstance::PickBestAnimLayer(bool bEquipped, const FGameplayTagContainer& CosmeticTags) const
{
	const FRPGAnimLayerSelectionSet& SetToQuery = (bEquipped ? EquippedAnimSet : UneuippedAnimSet);
	return SetToQuery.SelectBestLayer(CosmeticTags);
}

const FPlatformUserId URPGWeaponInstance::GetOwningUserId() const
{
...
void URPGWeaponInstance::ApplyDeviceProperties()
{
...
void URPGWeaponInstance::RemoveDeviceProperties()
{
	const FPlatformUserId UserId = GetOwningUserId();
...
void URPGWeaponInstance::OnDeathStarted(AActor* OwningActor)
{
