...
#include "RPGWeaponStateComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RPGWeaponStateComponent)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Gameplay_Zone, "Gameplay.Zone");

URPGWeaponStateComponent::URPGWeaponStateComponent(const FObjectInitializer& ObjectInitializer)
...
void URPGWeaponStateComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (APawn* Pawn = GetPawn<APawn>())
	{
		...
		{
			if (URPGRangedWeaponInstance* CurrentWeapon = Cast<URPGRangedWeaponInstance>(EquipmentManager->GetFirstInstanceOfType(URPGRangedWeaponInstance::StaticClass())))
			{
...
bool URPGWeaponStateComponent::ShouldShowHitAsSuccess(const FHitResult& Hit) const
{
...
bool URPGWeaponStateComponent::ShouldUpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext) const
{
...
void URPGWeaponStateComponent::ClientConfirmTargetData_Implementation(uint16 UniqueId, bool bSuccess, const TArray<uint8>& HitReplaces)
{
	for (int i = 0; i < UnconfirmedServerSideHitMarkers.Num(); i++)
	{
		FRPGServerSideHitMarkerBatch& Batch = UnconfirmedServerSideHitMarkers[i];
		if (Batch.UniqueId == UniqueId)
		{
			if (bSuccess && (HitReplaces.Num() != Batch.Markers.Num()))
			{
				...
				for (const FRPGScreenSpaceHitLocation& Entry : Batch.Markers)
				{
...
void URPGWeaponStateComponent::AddUnconfirmedServerSideHitMarkers(const FGameplayAbilityTargetDataHandle& InTargetData, const TArray<FHitResult>& FoundHits)
{
	FRPGServerSideHitMarkerBatch& NewUnconfirmedHitMarker = UnconfirmedServerSideHitMarkers.Emplace_GetRef(InTargetData.UniqueId);
	...
	if (APlayerController* OwnerPC = GetController<APlayerController>())
	{
		for (const FHitResult& Hit : FoundHits)
		{
			...
			{
				FRPGScreenSpaceHitLocation& Entry = NewUnconfirmedHitMarker.Markers.AddDefaulted_GetRef();
...
void URPGWeaponStateComponent::UpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext)
{
...
void URPGWeaponStateComponent::ActuallyUpdateDamageInstigatedTime()
{
...
double URPGWeaponStateComponent::GetTimeSinceLastHitNotification() const
{
	UWorld* World = GetWorld();
