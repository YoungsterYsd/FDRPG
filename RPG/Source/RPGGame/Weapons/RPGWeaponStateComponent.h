...
#include "RPGWeaponStateComponent.generated.h"
...
struct FRPGScreenSpaceHitLocation
{
...
struct FRPGServerSideHitMarkerBatch
{
	FRPGServerSideHitMarkerBatch() { }

	FRPGServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<FRPGScreenSpaceHitLocation> Markers;
...
UCLASS()
class URPGWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	URPGWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<FRPGScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FRPGScreenSpaceHitLocation> LastWeaponDamageScreenLocations;

	/** The unconfirmed hits */
	TArray<FRPGServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
};
